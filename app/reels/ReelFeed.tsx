...
import ReactionBar, { ReelReactionEmoji } from "@/components/reels/ReactionBar";
import aurora from "@/components/reels/AuroraMotion.module.css";
...
export default function ReelFeed({ refreshKey = 0 }: ReelFeedProps) {
  ...
  const [auroraReelId, setAuroraReelId] = useState<string | null>(null);
  ...
  const loadReels = useCallback(async () => {
    ...
    const [profilesResult, likesResult, savedResult, reactionsResult] = await Promise.all([
      userIds.length
        ? supabase
            .from("profiles")
    ...
    const myReactions = new Map<string, ReelReactionEmoji>();

    for (const item of (reactionsResult.data ?? []) as Array<{
      reel_id: string;
      user_id: string;
      emoji: ReelReactionEmoji;
    }>) {
      const current = reactionCounts.get(item.reel_id) ?? {};
      current[item.emoji] = (current[item.emoji] ?? 0) + 1;
      reactionCounts.set(item.reel_id, current);

      if (item.user_id === user.id) {
        myReactions.set(item.reel_id, item.emoji);
      }
    }
    ...
    const prepared = rows.map<ReelItem>((reel) => ({
      ...
      reactionCounts: reactionCounts.get(reel.id) ?? {},
      myReaction: myReactions.get(reel.id) ?? null,
    }));
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "reel_reactions" },
        () => void loadReels()
      )
      .subscribe();
  ...
  function launchAurora(reelId: string, emoji: ReelReactionEmoji) {
    const token = Date.now();
    setAuroraReelId(reelId);
    setReactionBurst({ reelId, emoji, token });

    window.setTimeout(() => {
      setAuroraReelId((current) => (current === reelId ? null : current));
    }, 1150);
  ...
  async function doubleTapLike(reel: ReelItem) {
    const token = Date.now();
    setDoubleTapHeart({ reelId: reel.id, token });
    setAuroraReelId(reel.id);

    window.setTimeout(() => {
      setDoubleTapHeart((current) =>
        current?.token === token ? null : current
      );
    }, 1050);

    window.setTimeout(() => {
      setAuroraReelId((current) => (current === reel.id ? null : current));
    }, 1150);
  ...
  async function chooseReaction(
    ...
    setBusy(reel.id, true);
    const previous = reel.myReaction;
    const remove = previous === emoji;

    if (!remove) {
      launchAurora(reel.id, emoji);
    }
    ...
    if (result.error) {
      console.error("Reel reaction error:", result.error);
      setReels((current) =>
        current.map((item) =>
          item.id === reel.id
            ? {
        ...
        {reels.map((reel) => {
          ...
          return (
            <article
              ...
              className={`reel-card ${
                auroraReelId === reel.id ? aurora.auroraActive : ""
              }`}
            >
              <div className="reel-video-shell">
                ...
                <div
                  className={`${aurora.auroraFrame} ${
                    auroraReelId === reel.id ? aurora.frameVisible : ""
                  }`}
                  aria-hidden="true"
                >
                  <span className={aurora.ribbonOne} />
                  <span className={aurora.ribbonTwo} />
                  <span className={aurora.ribbonThree} />
                </div>

                <AnimatePresence>
                  {reactionBurst?.reelId === reel.id && (
                    <motion.div
                      key={reactionBurst.token}
                      className={aurora.reactionBurst}
                      initial={{ opacity: 0, scale: 0.35, y: 42, rotate: -12 }}
                      animate={{ opacity: [0, 1, 1, 0], scale: [0.35, 1.3, 1, 0.7], y: [42, 0, -70, -145], rotate: [-12, 8, -4, 12] }}
                      exit={{ opacity: 0 }}
                ...
                <AnimatePresence>
                  {doubleTapHeart?.reelId === reel.id && (
                    <motion.div
                      key={doubleTapHeart.token}
                      className={aurora.doubleTapHeart}
                      initial={{ opacity: 0, scale: 0.2, rotate: -18 }}
                      animate={{ opacity: [0, 1, 1, 0], scale: [0.2, 1.35, 1, 1.8], rotate: [-18, 8, 0, 10] }}
                      exit={{ opacity: 0 }}
                      transition={{ duration: 0.95, times: [0, 0.28, 0.68, 1], ease: "easeOut" }}
                      aria-hidden="true"
                    >
                ...
                <aside className="reel-actions" aria-label="Acțiuni Reel">
                  ...
                  <ReactionBar
                    reelId={reel.id}
                    counts={reel.reactionCounts}
                    myReaction={reel.myReaction}
                    disabled={busy}
                    onSelect={(emoji) => chooseReaction(reel, emoji)}
                  />

                  <button
                    type="button"
                    onClick={() => setCommentsReelId(reel.id)}
