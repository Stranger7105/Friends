...
export default function GlobalCallOverlay() {
  ...
  if (!activeCall && error) {
    return (
      ...
      >
        <div style={{ fontWeight: 800 }}>Apel Friends</div>
  ...
  const statusText = incomingRinging
    ? "Te apelează"
    : outgoingRinging
      ? "Se apelează…"
      : accepted
      ...
      >
        ...
        >
          {activeCall.peerAvatarUrl ? (
            ...
            initials(activeCall.peerName) || "F"
          )}
        </div>
        ...
        <div style={{ minWidth: 0, flex: 1 }}>
          ...
          {accepted && (
            ...
            >
              Conexiunea audio se activează în M4.1B.
            </div>
          )}
        </div>

        <Phone size={20} color="#6ee7b7" />
      </div>
